...
$roles = Spatie\Permission\Models\Role::all()->sortByDesc('id')->toArray();
					...
					<div class="wt-registerformmain">
						<div class="wt-joinforms">
							<form method="POST" action="{{{ url('register/form-step1-custom-errors') }}}" class="wt-formtheme wt-formregister" @submit.prevent="checkStep1" id="register_form">
								...
								<fieldset class="wt-registerformgroup">
									<div class="wt-haslayout" v-if="step === 1" v-cloak>
										...
										<div id="step-1" class="form-group form-group-half">
											<input type="text" name="first_name" class="form-control test" placeholder="{{{ trans('lang.ph_first_name') }}}" v-bind:class="{ 'is-invalid': form_step1.is_first_name_error }" v-model="first_name">
								...
								<div id="step-2" class="wt-haslayout" v-if="step === 2" v-cloak>
									<fieldset class="wt-registerformgroup">
										<div class="wt-registerhead">
											<div class="wt-title">
												<h3>{{{ $reg_two_title }}}</h3>
										...
										<div class="form-group">
											<multiselect placeholder="Select skills" :multiple="true" :tagging="true" v-model="skills" track-by="id" label="title" :options="{{{json_encode($skills)}}}">
												<span slot="noResult">Oops! Please enter valid matches</span>
											</multiselect>

										</div>
										<div class="form-group">
											<multiselect placeholder="Select Categories" :multiple="true" :tagging="true" v-model="categories" track-by="id" label="title" :options="{{{json_encode($categories)}}}">
												<span slot="noResult">Oops! Please enter valid matches</span>
											</multiselect>

										</div>
										<div class="form-group">
											<upload-image :id="'banner_id'" :img_ref="'banner_ref'" :url="'{{url('freelancer/upload-temp-image')}}'" :name="'hidden_banner_image'">
											</upload-image>
										</div>
									</fieldset>
										...
										<ul class="wt-accordionhold wt-formaccordionhold accordion">
											...
											<li>
												<div class="wt-accordiontitle" id="headingOne" data-toggle="collapse" data-target="#collapseOne">
													<span class="wt-radio">
														<input id="wt-company-{{$key}}" type="radio" name="role" value="{{{ $role['role_type'] }}}" {{$role['name'] === 'freelancer' ? 'checked="true"' : ''}} v-model="user_role" v-on:change="selectedRole(user_role)">
									...
									<fieldset class="wt-termsconditions" style="margin-bottom:20px">
										<div class="wt-checkboxholder">
											...
											<a id="continue" href="javascript:void(0)" @click.prevent="onboardDirectly('{{ trans('lang.email_not_config') }}')" class="wt-btn">{{{ trans('lang.continue') }}}</a>
								...
								<div id="captcha" class="g-recaptcha" data-sitekey="{{env('GOOGLE_RECAPTCHA_KEY')}}" style="display:none"></div>
							...
							<div id="gotodashboard" class="wt-gotodashboard" v-if="step === 4" v-cloak>
								<ul class="wt-joinsteps">
									<li class="wt-done-next"><a href="javascrip:void(0);"><i class="fa fa-check"></i></a></li>
									<li class="wt-done-next"><a href="javascrip:void(0);"><i class="fa fa-check"></i></a></li>
									<li class="wt-done-next"><a href="javascrip:void(0);"><i class="fa fa-check"></i></a></li>
