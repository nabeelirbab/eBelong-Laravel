								...
								<tbody>
									@foreach ($orders as $order)
										@php 
											...
											$amount = '';
											$attachment = '';
											if ($order->type == 'job' || $order->type == 'project') {
												$proposal = App\Proposal::find($order->product_id);
												$is_featured = isset($proposal->job->is_featured) ? $proposal->job->is_featured: '';
												$title = isset($proposal->job->title) ? $proposal->job->title : ''; 
												$amount = isset($proposal->amount) ? $proposal->amount:'';
