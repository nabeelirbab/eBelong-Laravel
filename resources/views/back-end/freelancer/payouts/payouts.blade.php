...
@section('content')
    <section class="wt-haslayout wt-dbsectionspace">
        <div class="row">
            <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12 col-xl-10 float-right" id="invoice_list">
                            ...
                            <table class="wt-tablecategories">
                                <thead>
                                    <tr>
                                        ...
                                        <th>{{ trans('lang.receipt_id') }}</th>
                                        <th>{{ trans('lang.project_name_service_name') }}</th>
                                    </tr>
                                </thead>
                                @if($payouts->count() > 0)
                                    <tbody>
                                        @foreach ($payouts as $key => $payout)
                                            <tr>
                                                ...
                                                <td>{{ $payout->status != "" ? strtoupper(str_replace("_"," ",$payout->status)) : '' }}</td>
                                                <td>{{ $payout->refrence_no }}</td>
                                                <td>{{ $payout->projects_ids }}</td>
                                            </tr>
                                        @endforeach
                                    </tbody>
                                @endif
