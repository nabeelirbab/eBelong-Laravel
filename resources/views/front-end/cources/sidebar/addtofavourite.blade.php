...
<div class="wt-clicksavearea">
    <span>{{{trans("lang.cource_id")}}}: {{{$cource->code}}}</span>
        ...
        <div class="wt-clicksavearea">
            <a href="javascript:void(0);" id="add-{{$cource->id}}" class="wt-clicksavebtn" style="{{ $course_saved ? 'display: none;' : '' }}"
                @click.prevent="add_wishlist('add-{{$cource->id}}', {{ $cource->id }}, 'saved_cources', {{$seller_id}}, '{{{trans('lang.saved')}}}','remove-{{$cource->id}}')" v-cloak>
