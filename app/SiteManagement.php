...
class SiteManagement extends Model
{
    ...
    /**
     * Save Freelancer commision settings
     *
        ...
        if (!empty($commision_settings)) {
            ...
            foreach ($commision_settings as $key => $setting) {
                $commision_settings_array[$key]['commision'] = $setting['commision'];
                 $commision_settings_array[$key]['emp_commision'] = $setting['emp_commision'];
    ...
    /**
     * Save Employer commision settings
     *
     * @param string $commision_settings commision settings
     *
     * @return \Illuminate\Http\Response
     */
    public function saveEmpCommisionSettings($commision_settings)
    {
        $commision_settings_array = array();
        if (!empty($commision_settings)) {
            $currency = !empty($commision_settings) && !empty($commision_settings[0]['currency']) ? $commision_settings[0]['currency'] : '';
            ...
            $existing_data = SiteManagement::getMetaValue('emp_commision');
            if (!empty($existing_data)) {
                DB::table('site_managements')->where('meta_key', '=', 'commision')->delete();
                Helper::changeEnv(
                    [
            ...
            DB::table('site_managements')->insert(
                [
                    'meta_key' => 'emp_commision', 'meta_value' => serialize($commision_settings_array),
                    "created_at" => Carbon::now(), "updated_at" => Carbon::now()
                ]
            );
            Helper::changeEnv(
    ...
    public function savePaymentSettings($request)
    {
        ...
        $payment_settings[0]['paypal_client_id'] = !empty($paypal_client_id) ? $paypal_client_id : '';
    ...
    public function saveBankTransfarSettings($request)
    {
        $banktransfar_key = $request['banktransfar_key'];
        $banktransfar_secret = $request['banktransfar_secret'];
        $api_endpoint = $request['api_endpoint'];
        $enable_sandbox = $request['enable_sandbox'];
        $remitter_identification_type = $request['remitter_identification_type'];
        $remitter_identification_number = $request['remitter_identification_number'];
        $remitter_country_code = $request['remitter_country_code'];
        $remitter_address = $request['remitter_address'];
        $remit_purpose_code = $request['remit_purpose_code'];
        $remitter_city = $request['remitter_city'];
        $remitter_postcode = $request['remitter_postcode'];
        $payment_settings = array();
        $payment_settings[0]['banktransfar_key'] = !empty($banktransfar_key) ? $banktransfar_key : '';
        $payment_settings[0]['banktransfar_secret'] = !empty($banktransfar_secret) ? $banktransfar_secret : '';
        $payment_settings[0]['api_endpoint'] = !empty($api_endpoint) ? $api_endpoint : '';
        $payment_settings[0]['enable_sandbox'] = !empty($enable_sandbox) ? $enable_sandbox : '';
        $payment_settings[0]['remitter_identification_type'] = !empty($remitter_identification_type) ? $remitter_identification_type : '';
        $payment_settings[0]['remitter_identification_number'] = !empty($remitter_identification_number) ? $remitter_identification_number : '';
        $payment_settings[0]['remitter_country_code'] = !empty($remitter_country_code) ? $remitter_country_code : '';
        $payment_settings[0]['remitter_address'] = !empty($remitter_address) ? $remitter_address : '';
        $payment_settings[0]['remit_purpose_code'] = !empty($remit_purpose_code) ? $remit_purpose_code : '';
        $payment_settings[0]['remitter_city'] = !empty($remitter_city) ? $remitter_city : '';
        $payment_settings[0]['remitter_postcode'] = !empty($remitter_postcode) ? $remitter_postcode : '';

        if (!empty($payment_settings)) {
            $existing_payment_settings = SiteManagement::getMetaValue('banktransfar_settings');
            if (!empty($existing_payment_settings)) {
                DB::table('site_managements')->where('meta_key', '=', 'banktransfar_settings')->delete();
                Helper::changeEnv(
                    [
                        'banktransfar_key' => "",
                        'banktransfar_secret' => "",
            ...
            DB::table('site_managements')->insert(
                [
                    'meta_key' => 'banktransfar_settings', 'meta_value' => serialize($payment_settings),
                    "created_at" => Carbon::now(), "updated_at" => Carbon::now()
                ]
            );
            Helper::changeEnv(
                [
                    'banktransfar_key' => $banktransfar_key,
                    'banktransfar_secret' => $banktransfar_secret,
                    'api_endpoint' => $api_endpoint,
                    'enable_sandbox' => $api_endpoint,
                    'remitter_identification_type' => "",
                    'remitter_identification_number' => "",
                    'remitter_country_code' => "",
                    'remitter_address' => "",
