...
class SiteManagementController extends Controller
{
    ...
    public function settings()
    {
        ...
        $emp_commision = !empty($payout_settings[0]['emp_commision']) ? $payout_settings[0]['emp_commision'] : null;
        ...
        $banktransfar_settings = $this->settings::getMetaValue('banktransfar_settings');
        
        $banktransfar_key = !empty($banktransfar_settings) && !empty($banktransfar_settings[0]['banktransfar_key']) ? $banktransfar_settings[0]['banktransfar_key'] : '';

        $banktransfar_secret = !empty($banktransfar_settings) && !empty($banktransfar_settings[0]['banktransfar_secret']) ? $banktransfar_settings[0]['banktransfar_secret'] : '';

        $api_endpoint = !empty($banktransfar_settings) && !empty($banktransfar_settings[0]['api_endpoint']) ? $banktransfar_settings[0]['api_endpoint'] : '';

        $remitter_identification_type = !empty($banktransfar_settings) && !empty($banktransfar_settings[0]['remitter_identification_type']) ? $banktransfar_settings[0]['remitter_identification_type'] : '';

        $remitter_identification_number = !empty($banktransfar_settings) && !empty($banktransfar_settings[0]['remitter_identification_number']) ? $banktransfar_settings[0]['remitter_identification_number'] : '';

        $remitter_country_code = !empty($banktransfar_settings) && !empty($banktransfar_settings[0]['remitter_country_code']) ? $banktransfar_settings[0]['remitter_country_code'] : '';

        $remitter_address = !empty($banktransfar_settings) && !empty($banktransfar_settings[0]['remitter_address']) ? $banktransfar_settings[0]['remitter_address'] : '';

        $remit_purpose_code = !empty($banktransfar_settings) && !empty($banktransfar_settings[0]['remit_purpose_code']) ? $banktransfar_settings[0]['remit_purpose_code'] : '';

        $remitter_city = !empty($banktransfar_settings) && !empty($banktransfar_settings[0]['remitter_city']) ? $banktransfar_settings[0]['remitter_city'] : '';

        $remitter_postcode = !empty($banktransfar_settings) && !empty($banktransfar_settings[0]['remitter_postcode']) ? $banktransfar_settings[0]['remitter_postcode'] : '';
        ...
        if (file_exists(resource_path('views/extend/back-end/admin/settings/index.blade.php'))) {
            return view(
                ...
                compact(
                    ...
                    'sender_avatar', 'title', 'email', 'logo', 'commision', 'emp_commision',
    ...
    /**
     * Store FreeLancer commision settings
     *
     * @param mixed $request get req attributes
     *
     * @access public
    ...
    public function storeBankTransfarSettings(Request $request)
    {
        $server = Helper::worketicIsDemoSiteAjax();
        if (!empty($server)) {
            $response['type'] = 'error';
        ...
        $this->validate(
            $request, 
            [
                'banktransfar_key' => 'required',
                'banktransfar_secret' => 'required',
                'api_endpoint' => 'required',
                "remitter_identification_type"=>"required",
                "remitter_identification_number"=>"required",
                "remitter_country_code"=>"required",
                "remitter_address"=> "required",
                "remit_purpose_code"=>"required",
                "remitter_city"=>"required",
                "remitter_postcode"=>"required"
            ]
        );
        $json = array();
        if (!empty($request)) {

            $default_settings = $this->settings->saveBankTransfarSettings($request);
