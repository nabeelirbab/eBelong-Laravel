...
public function employerHourlyPaymentProcess($id)
    {
        if (Auth::user() && !empty($id)) {
            if (Auth::user()) {
                $user_id = Auth::user()->id;
             ...
             //  dd($payout_settings[0]['commision']);
                ...
                if(!empty($payout_settings) && !empty($payout_settings[0]['commision'] || $payout_settings[0]['commision'] == 0 || $payout_settings[0]['commision'] == 'Null'))
                {
                   if($job->project_type == 'fixed') {
                    ...
                    $total_amount = $proposal->amount + $commision_amount;
                      ...
                      $hourly_project_amount_per_week =  $proposal->amount * $job->no_of_hours;
                      $total_amount =  $hourly_project_amount_per_week + ($hourly_project_amount_per_week *  ( $commision_amount / 100) ) ;
                      $hourly_project_amount_per_week = 91.75;
                }
                ...
                if (file_exists(resource_path('views/extend/back-end/employer/jobs/hourly_billing_checkout.blade.php'))) {
                    return view(
                        'extend.back-end.employer.jobs.hourly_billing_checkout',
                        compact(
                            'job',
                            'freelancer_name',
                            'profile_image',
                    ...
                    return view(
                        'back-end.employer.jobs.hourly_billing_checkout',
                        compact(
                            'job',
                            'freelancer_name',
                            'profile_image',
    ...
    {
        $payouts =  Payout::where('employee_id', Auth::user()->id)->paginate(10);
